...
## Full-course release measurements

The later release run used full Chromium headless 153.0.8010.12 with ANGLE/NVIDIA Quadro P2000/D3D11 on Windows x64, i5-12600 (12 logical CPUs), approximately 32 GiB RAM. It exercised ten real 30-second races: all five stages with 100 entries and all five breeds at 1440x900, then all five at a 390x844 mobile viewport. Auto quality was retained. This run includes the final mountain, vegetation, camera and wake changes.

The saved intervals contain 24,394 actual frames over 287.0305 seconds: 84.99 weighted FPS, lowest reported two-second interval 84 FPS. Desktop stage averages are 84.90–85.01 FPS; mobile viewport averages are 84.97–85.01 FPS. Forest Lake adapted to low on desktop; the other desktop stages stayed high. Every mobile viewport stage used low. All ten races produced complete 100-entry rankings. See `docs/evidence/performance-latest.json` for each sample, hardware, quality tier and stage, and `docs/evidence/performance/` for mid-race screenshots.
