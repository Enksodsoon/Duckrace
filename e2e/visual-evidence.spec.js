...
test('capture complete desktop and mobile screen evidence', async ({ page }) => {
  ...
  const ready = async () => {
    await expect(page.locator('.scene-layer')).toHaveAttribute('data-scene-state', 'ready', { timeout: 60_000 });
    await page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))));
  };
  const capture = async name => { await ready(); await page.screenshot({ path: `${folder}/${name}.png`, fullPage: true }); };
  ...
  await page.getByLabel('Race entries', { exact: true }).fill('Maple\nWillow\nRiver\nSnow\nPebble\nCobalt');
  ...
  for (const stage of ['Forest Lake', 'Mountain River', 'Lotus Pond', 'Sunset Marsh', 'Village Canal']) {
  ...
  await expect(page.locator('.scene-layer')).toHaveAttribute('data-phase', 'racing', { timeout: 60_000 });
  await capture('race-desktop');
  await expect(page.getByRole('heading', { name: 'Results', exact: true })).toBeVisible({ timeout: 30_000 });
  await capture('results-desktop');
  await page.getByRole('button', { name: 'Settings', exact: true }).first().click();
  await capture('settings-desktop');
  await page.setViewportSize({ width: 390, height: 844 });
  for (const [screen, button] of [['home', 'Home'], ['setup', 'Play Race'], ['garage', 'Duck Garage'], ['stages', 'Stages'], ['results', 'Results'], ['settings', 'Settings']]) {
...
test('capture stage thumbnails directly from the 3D scene', async ({ page }) => {
  test.skip(!process.env.CAPTURE_STAGE_ASSETS, 'Only author thumbnails from the local development scene.');
