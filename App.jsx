...
function splitEntries(text) {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    const value = typeof parts[i] === "string" ? parts[i].trim() : "";
...
function removeManyOccurrences(list, values) {
  ...
  const toRemove = Array.isArray(values) ? values : [];
  for (let i = 0; i < toRemove.length; i += 1) {
    const index = remaining.indexOf(toRemove[i]);
    if (index !== -1) remaining.splice(index, 1);
  }
  return remaining;
}

function placeLabelFromIndex(index) {
  const n = index + 1;
  const mod10 = n % 10;
  const mod100 = n % 100;
  let suffix = "th";
...
function getEliminationTargets(winnersByPlace, eliminationPlaces) {
  const targets = [];
  const selected = Array.isArray(eliminationPlaces) ? eliminationPlaces : [];
  const winners = Array.isArray(winnersByPlace) ? winnersByPlace : [];
  for (let i = 0; i < selected.length; i += 1) {
    const winner = winners[selected[i]];
...
function getArenaMetrics(count, audience) {
  ...
  if (audience) {
    ...
    return {
      topPct, bottomPct, laneStepPct, duckScale,
  ...
  return {
    ...
    startX: 12, finishX: 90,
  };
}

function pillStyle(bg="#fff", color="#334155", border="#e2e8f0") {
  return {
...
function CountdownOverlay({ value, audience = false }) {
  if (value === null) return null;
  return (
    <div style={{
      position:"absolute", inset:0, zIndex:40, display:"flex", alignItems:"center", justifyContent:"center",
      borderRadius:32, background:"rgba(2,6,23,0.45)", backdropFilter:"blur(2px)"
    }}>
      <div style={{display:"flex", flexDirection:"column", alignItems:"center", gap:8}}>
        <div style={{fontSize: audience ? 120 : 84, fontWeight:900, color:"#fff", lineHeight:1}}>{value}</div>
        <div style={{...pillStyle("rgba(255,255,255,0.1)", "#fff", "rgba(255,255,255,0.2)")}}>Race starts now</div>
      </div>
    </div>
  );
}

function WinnerBanner({ winnerName, show, audience = false }) {
  if (!show || !winnerName) return null;
  return (
    <div style={{position:"absolute", left:0, right:0, top: audience ? 20 : 16, zIndex:30, display:"flex", justifyContent:"center", pointerEvents:"none"}}>
      <div style={{
        margin:"0 16px", padding:"16px 24px", background:"rgba(255,255,255,0.88)", border:"1px solid #fde68a",
        borderRadius:32, boxShadow:"0 24px 80px rgba(15,23,42,0.18)", maxWidth: audience ? 980 : 700, width:"fit-content"
      }}>
        <div style={{textAlign:"center"}}>
          ...
          <div style={{marginTop:4, fontWeight:900, color:"#0f172a", fontSize: audience ? 58 : 34, lineHeight:1.05}}>{winnerName}</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{position:"relative", width:80, height:64}}>
        <div style={{
          position:"absolute", left:12, top:20, width:48, height:36, borderRadius:999,
        ...
        <div style={{
          position:"absolute", left:44, top:4, width:32, height:32, borderRadius:999,
          border:`1px solid ${winner ? "#fcd34d" : isPodium ? "#cbd5e1" : "#e2e8f0"}`,
          background:`linear-gradient(180deg, ${variant.palette.bodyA} 0%, ${variant.palette.bodyB} 100%)`
        }}>
        ...
        {place > -1 ? (
          <div style={{
            position:"absolute", left:-4, top:-4, borderRadius:999, border:`1px solid ${placeColors(place).border}`,
            padding:"2px 6px", fontSize:10, fontWeight:700, background:placeColors(place).chipBg, color:placeColors(place).chipText
          }}>{placeLabelFromIndex(place)}</div>
        ) : null}
      </div>
    </div>
  );
...
function RaceArena({ racers, progress, placements, isRacing, showBurst, countdownValue, audience = false, avatarSeed = 0, motionTime = 0 }) {
  ...
  return (
    <div style={{position:"relative", width:"100%", overflow:"hidden", borderRadius:32, border:"1px solid #e2e8f0", background:"rgba(255,255,255,0.9)", boxShadow:"0 20px 60px rgba(15,23,42,0.06)"}}>
      ...
      <WinnerBanner winnerName={winnerName} show={Boolean(winnerName && !isRacing && countdownValue === null)} audience={audience} />
      <CountdownOverlay value={countdownValue} audience={audience} />
      <div style={{position:"relative", ...metrics.heightStyle}}>
        <div style={{position:"absolute", top:16, left: audience ? 24 : 20, zIndex:10}}><div style={pillStyle()}>Start</div></div>
        <div style={{position:"absolute", top:16, right: audience ? 24 : 20, zIndex:10}}><div style={pillStyle("#fffbeb", "#b45309", "#fde68a")}>Finish</div></div>
        ...
        {racers.map((name, index) => {
          ...
          const colors = place > -1 ? placeColors(place) : null;
          const variant = variants[index];
          return (
            <div key={`${name}-${index}`}>
              <div style={{position:"absolute", left:0, right:0, top:`${yPct}%`, borderTop:`1px solid ${place > -1 ? colors.border : "rgba(226,232,240,0.8)"}`}} />
              <div style={{
                position:"absolute", top:`${yPct}%`, left:`calc(${metrics.startX}% - 0.75rem)`, transform:"translateY(-50%)",
                maxWidth: audience ? "18rem" : "14rem", padding:"6px 10px", borderRadius:999, border:`1px solid ${place > -1 ? colors.border : "#e2e8f0"}`,
                background: place > -1 ? colors.bg : "rgba(255,255,255,0.85)", backdropFilter:"blur(4px)", zIndex:10,
                fontSize:metrics.labelSize, fontWeight:500, color:"#334155", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"
              }}>{name}</div>
              <div style={{position:"absolute", top:`${yPct}%`, right:audience ? 20 : 16, transform:"translateY(-50%)", zIndex:10, color:"#64748b", fontWeight:600, fontSize:metrics.percentSize}}>
                {Math.round(pct)}%
              </div>
              {place > -1 ? (
                <div style={{
                  position:"absolute", top:`${yPct}%`, left: audience ? 16 : 14, transform:"translateY(-50%)", zIndex:10,
                  borderRadius:999, border:`1px solid ${colors.border}`, padding:"2px 8px", fontSize:10, fontWeight:700,
              ...
              {winner && showBurst ? (
                <>
                  ...
                  {Array.from({ length: 10 }).map((_, burstIndex) => (
                    <div key={`burst-${index}-${burstIndex}`} style={{
                      position:"absolute", top:`${yPct}%`, left:`${metrics.finishX}%`, zIndex:10, width:`${28 + (burstIndex % 3) * 12}px`, height:2,
                      transform:`translateY(-50%) rotate(${burstIndex * 36}deg)`, transformOrigin:"left center",
                      background:"linear-gradient(to right, rgba(252,211,77,0.85), transparent)", borderRadius:999
                    }} />
                  ))}
                </>
              ) : null}
        ...
        {Array.from({ length: podiumSlots }).map((_, index) => {
          ...
          return (
            <button
              ...
              style={{
                borderRadius:999, padding:"8px 12px", fontSize:14, fontWeight:500, cursor:"pointer",
                border: active ? "1px solid #0f172a" : "1px solid #e2e8f0",
                background: active ? "#0f172a" : "#fff",
                color: active ? "#fff" : "#334155"
              }}
            >
              Eliminate {placeLabelFromIndex(index)}
            </button>
          );
        })}
      </div>
      <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
        <button type="button" onClick={onClear} style={btnStyle("outline")}>None</button>
        <button type="button" onClick={onFirstOnly} style={btnStyle("outline")}>1st only</button>
        <button type="button" onClick={onAll} style={btnStyle("outline")}>All podium places</button>
      </div>
    </div>
  );
}
...
function LabeledInput({ label, children, hint }) {
  return (
    <div style={{display:"grid", gap:8}}>
      <label style={{fontWeight:600, color:"#0f172a"}}>{label}</label>
      {children}
...
export default function App() {
  ...
  function playToneSequence(tones, baseDelay = 0) {
    const ctx = getAudioContext();
    if (!ctx) return;
    try {
      const start = ctx.currentTime + baseDelay;
      for (let i = 0; i < tones.length; i += 1) {
        ...
        osc.stop(start + tone.at + tone.duration + 0.03 + i * 0.001);
      }
    } catch {
      setAudioBlocked(true);
    }
  ...
  function storeResults(winners) {
    ...
    const summary = winners.map((name, index) => `${placeLabelFromIndex(index)} ${name}`).join(" • ");
    setLastResults((prev) => [summary, ...prev.filter((item) => item !== summary)].slice(0, 8));
  }

  function maybeEliminateWinners(winnersByPlace) {
    const targets = getEliminationTargets(winnersByPlace, eliminationPlaces);
  ...
  function runRace() {
    ...
    const durationMs = Math.max(3, duration[0]) * 1000;
    ...
    const frame = (nowValue) => {
      ...
      const nextProgress = plans.map((plan, index) => {
        ...
        const monotonic = Math.max(previous, computed);
        return Math.min(monotonic, plan.target);
      });
        ...
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"end", gap:16, flexWrap:"wrap"}}>
          <div>
            <div style={{display:"flex", gap:8, flexWrap:"wrap", marginBottom:8}}>
              <span style={pillStyle("#fff", "#475569", "#e2e8f0")}>Shared race track</span>
              <span style={pillStyle("#fff", "#475569", "#e2e8f0")}>Procedural duck avatars</span>
            </div>
          ...
          <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
            <span style={pillStyle("#fff", "#334155", "#e2e8f0")}>{parsedEntries.length} entries</span>
            <span style={pillStyle("#fff", "#334155", "#e2e8f0")}>podium {podiumSlots}</span>
          </div>
        </div>
        ...
        <div style={{display:"grid", gap:24, gridTemplateColumns:"380px minmax(0,1fr)"}}>
          <div style={cardStyle()}>
            ...
            <div style={{padding:22, display:"grid", gap:20}}>
              <LabeledInput label="Names / groups / case numbers">
              ...
              <div style={{display:"grid", gridTemplateColumns:"repeat(3, minmax(0,1fr))", gap:8}}>
                <LabeledInput label="Start"><input value={numberStart} onChange={(e) => setNumberStart(e.target.value)} style={inputStyle()} /></LabeledInput>
                <LabeledInput label="End"><input value={numberEnd} onChange={(e) => setNumberEnd(e.target.value)} style={inputStyle()} /></LabeledInput>
                <LabeledInput label="Prefix"><input value={prefix} onChange={(e) => setPrefix(e.target.value)} style={inputStyle()} /></LabeledInput>
              </div>

              <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                <button onClick={generateNumbers} style={btnStyle("secondary")}><Hash size={16} style={{marginRight:6, verticalAlign:"text-bottom"}} />Generate</button>
                <button onClick={() => setEntriesText(SAMPLE)} style={btnStyle("outline")}>Sample</button>
                <button onClick={() => setEntriesText("")} style={btnStyle("outline")}>Clear</button>
              </div>

              <div style={{border:"1px solid #e2e8f0", borderRadius:24, padding:16, display:"grid", gap:16}}>
                <div style={{display:"grid", gap:10}}>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    <span style={{fontWeight:600, color:"#0f172a"}}>Race duration</span>
                    <span style={{fontSize:14, color:"#334155"}}>{duration[0]} sec</span>
                  </div>
                  <Slider min={3} max={12} step={1} value={duration} onValueChange={setDuration} />
                </div>

                <LabeledInput label="Podium size" hint="Choose how many ranked winners to show. Maximum is the number of entries.">
                ...
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:16}}>
                  <div>
                    <div style={{fontWeight:600, color:"#0f172a"}}>Shuffle entries</div>
                    <div style={{fontSize:12, color:"#64748b"}}>Random order for each race</div>
                  </div>
                  <Switch checked={shuffleBeforeRace} onCheckedChange={setShuffleBeforeRace} />
                </div>

                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:16}}>
                  <div>
                    <div style={{fontWeight:600, color:"#0f172a"}}>Sound effects</div>
                    <div style={{fontSize:12, color:"#64748b"}}>Start and finish tones</div>
                  </div>
                  <Switch checked={soundEnabled && !audioBlocked} onCheckedChange={setSoundEnabled} disabled={audioBlocked} />
                </div>

                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:16}}>
                  <div>
                    <div style={{fontWeight:600, color:"#0f172a"}}>Reroll avatar style each round</div>
                    <div style={{fontSize:12, color:"#64748b"}}>Off = same duck style per group name. On = new style every race.</div>
                  </div>
                  <Switch checked={rerollAvatarsEachRound} onCheckedChange={setRerollAvatarsEachRound} />
                </div>
              </div>

              <div style={{border:"1px solid #e2e8f0", borderRadius:24, padding:16, display:"grid", gap:8}}>
                <div style={{fontWeight:600, color:"#0f172a"}}>Eliminate after each round</div>
              ...
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                <button onClick={startRaceWithCountdown} disabled={!parsedEntries.length || isRacing || countdownValue !== null} style={btnStyle("primary")}>
                  <Play size={16} style={{marginRight:6, verticalAlign:"text-bottom"}} />
                  {countdownValue !== null ? "Counting..." : isRacing ? "Racing..." : "Start race"}
                </button>
                <button onClick={instantPick} disabled={!parsedEntries.length || isRacing || countdownValue !== null} style={btnStyle("secondary")}>
                  <Shuffle size={16} style={{marginRight:6, verticalAlign:"text-bottom"}} />Instant pick
                </button>
              </div>

              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                <button onClick={resetVisual} style={btnStyle("outline")}><RotateCcw size={16} style={{marginRight:6, verticalAlign:"text-bottom"}} />Reset stage</button>
                <button onClick={toggleAudienceMode} style={btnStyle("outline")}>
                  ...
                  {podiumWinners.length ? (
                    <div style={{display:"grid", gap:12}}>
                      {podiumWinners.map((item) => {
                        const colors = placeColors(item.place);
                        return (
                          <div key={`${item.place}-${item.name}`} style={{border:`1px solid ${colors.border}`, background:colors.bg, borderRadius:24, padding:16}}>
                            <div style={{display:"flex", alignItems:"center", gap:12}}>
                              ...
                              <div style={{flex:1, minWidth:0}}>
                                <div style={{fontSize:14, color:"#64748b"}}>{placeLabelFromIndex(item.place)}</div>
                                <div style={{fontSize:24, fontWeight:800, color:"#0f172a", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{item.name}</div>
                              </div>
                              <span style={pillStyle(colors.chipBg, colors.chipText, colors.border)}>{placeLabelFromIndex(item.place)}</span>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div style={{display:"grid", gap:8}}>
                      {lastResults.map((item, index) => (
                        <div key={`${item}-${index}`} style={{display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, border:"1px solid #e2e8f0", borderRadius:16, padding:"10px 12px"}}>
                          <span style={{whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", color:"#334155", fontWeight:500}}>{item}</span>
                          <span style={pillStyle("#fff","#334155","#e2e8f0")}>#{index + 1}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
          ...
          <div style={{maxWidth:1800, margin:"0 auto", height:"100%", display:"grid", gap:16}}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:16}}>
              <div>
                ...
                <div style={{fontSize:14, color:"#cbd5e1"}}>Procedurally generated duck avatars stay deterministic for each group and keep the app light.</div>
              </div>
              <div style={{display:"flex", gap:8}}>
                <button onClick={startRaceWithCountdown} disabled={!parsedEntries.length || isRacing || countdownValue !== null} style={btnStyle("light")}>
                  <Play size={16} style={{marginRight:6, verticalAlign:"text-bottom"}} />
                  {countdownValue !== null ? "Counting..." : isRacing ? "Racing..." : "Start"}
                </button>
                <button onClick={toggleAudienceMode} style={btnStyle("secondary")}><Minimize size={16} style={{marginRight:6, verticalAlign:"text-bottom"}} />Close</button>
              </div>
            </div>
                  ...
                  podiumWinners.map((item) => {
                    ...
                    return (
                      <div key={`overlay-${item.place}-${item.name}`} style={{border:`1px solid ${colors.border}`, background:colors.bg, borderRadius:22, padding:"12px 16px", boxShadow:"0 10px 30px rgba(15,23,42,0.16)", backdropFilter:"blur(8px)"}}>
                        <div style={{fontSize:12, fontWeight:700, letterSpacing:"0.18em", textTransform:"uppercase", color:"#64748b"}}>{placeLabelFromIndex(item.place)}</div>
